...
function getAbsolutePageUrl (dataFilePath, presskit) {
  ...
  const relativePath = path.posix.relative(
    normalizePathSeparators(buildFolder),
    normalizePathSeparators(htmlFilePath)
  )
...
function getImagesFolder (dataFile) {
  ...
  const dataFileFolder = stat.isFile() ? fsPath.dirname(dataFile) : dataFile

  return fsPath.join(dataFileFolder, imagesFolderName)
}
...
function getImages (source) {
  if (!fs.existsSync(source)) {
    console.warn(`No images found for "${source}"`, '🤔')
  ...
  sfs.findAllFiles(source)
    .forEach(filePathWithSource => {
      ...
      // Ignore favicon.
      if (basename === 'favicon.ico') return

      // And put in the correct category.
      if (basename.startsWith('header')) {
        images.header = f
      } else if (basename.startsWith('logo')) {
        images.logos.push(f)
      } else {
