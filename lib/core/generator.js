...
function findData (entryPoint) {
  ...
  getSelectedDataFiles(files).forEach(file => {
    try {
      ...
      if (ext === '.json') {
        console.error(`There was an error while loading \`${file}\`. Check that your JSON/JSONC file is valid.`)
        console.error('Check commas, comments, and required fields like "type" and "title".')
      } else {
...
function isDataFile (filename) {
  ...
  return filenameWithoutExt === 'data' && (ext === '.xml' || ext === '.json')
}

function getSelectedDataFiles (files) {
...
module.exports = {
  generate,
  __countDataFiles: countDataFiles,
  __isDataFile: isDataFile,
  __getSelectedDataFiles: getSelectedDataFiles,
  __startWatcher: startWatcher
}
